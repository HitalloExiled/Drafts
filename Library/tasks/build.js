...
function compileTS(isWatching) {
    ...
    switchers.push("false");
    let compile = (step, tsconfig, options) => log(step, ChildProcess.exec(`tsc -p ${tsconfig} ${options.join(" ")}`));
...
function copyStaticFiles(isWatching) {
    let targets = [
    ...
    targets.forEach(target => log(`Copy static files of ${target.module} modules from ${target.in} to ${target.out}`, ChildProcess.exec(`cpx ${target.in}/**/*.{d.ts,html,css} ${target.out} ${isWatching ? "--watch" : ""}`)));
}
execute();
